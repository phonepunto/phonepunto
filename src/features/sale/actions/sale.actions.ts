...
export async function addSalePaymentAction(
  ...
  try {
    ...
    if (!saleId) return { success: false, error: 'ID de venta requerido.' };
    if (!type || !['efectivo', 'transferencia'].includes(type)) {
      return { success: false, error: 'Método de pago inválido.' };
    }
    if (typeof amount !== 'number' || amount <= 0) {
      return { success: false, error: 'Monto de pago debe ser mayor a 0.' };
    }

    return await db.transaction(async (tx) => {
      const payment = await saleRepository.addPayment(saleId, type, amount, tx);
      ...
      await recordAuditLog(caller.id, 'CREAR', 'SALE_PAYMENT', payment.id, auditDetail, tx);
