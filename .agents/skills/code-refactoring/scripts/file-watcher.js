...
/**
 * Send desktop notification (DISABLED - using Claude Chat only)
 */
function sendNotification(title, message) {
  // Desktop notifications disabled - we use Claude Code Chat instead
  return;
}

/**
 * Write alert to JSON file for Claude Code Chat notifications
 */
function writeAlertToClaudeChat(filePath, lines, level, lineGrowth) {
  try {
...
function handleFileChange(eventType, filePath) {
  ...
  if (eventType === 'change' && problematicFiles.has(filePath)) {
    ...
    if (timeSinceLastReminder > reminderInterval || significantGrowth) {
      ...
      console.log(`${colors.cyan}   Ask Claude: "Help me refactor ${relativePath}"${colors.reset}`);
      console.log('');

      // Write alert to JSON file for Claude Code Chat notifications
      writeAlertToClaudeChat(relativePath, analysis.lines, tracked.level, significantGrowth ? lineGrowth : null);
