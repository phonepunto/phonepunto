...
test('no React warnings', async ({ page }) => {
  ...
  const criticalWarnings = reactWarnings.filter((w) => w.includes('Each child in a list should have a unique') || w.includes('Cannot update a component') || w.includes("Can't perform a React state update"));
