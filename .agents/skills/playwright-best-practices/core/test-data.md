...
export const test = base.extend<DataFixtures>({
  ...
  testProducts: async ({}, use) => {
    const products = [createProduct({ name: 'Test Product 1' }), createProduct({ name: 'Test Product 2' }), createProduct({ name: 'Test Product 3' })];
    await use(products);
  },
});
    ...
    async ({}, use) => {
      ...
      await use({
        ...
        seed: async (table: string, data: object) => {
          ...
          const result = await client.query(`INSERT INTO ${table} (${keys.join(', ')}) VALUES (${placeholders.join(', ')}) RETURNING *`, values);
