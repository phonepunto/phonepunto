...
export default defineConfig({
  reporter: process.env.CI ? [['dot'], ['html', { open: 'never' }], ['junit', { outputFile: 'results/junit.xml' }], ['github']] : [['list'], ['html', { open: 'on-failure' }]],
});
```
...
class NotificationReporter implements Reporter {
  ...
  async onEnd(result: FullResult) {
    ...
    const message = [`Tests ${status}`, `Passed: ${this.passed} | Failed: ${this.failed} | Skipped: ${this.skipped}`, `Duration: ${(result.duration / 1000).toFixed(1)}s`];
